...
訓練表 = fread(paste0(輸入目錄, "/quaterfinal_gy_cmp_training_traveltime.txt"), col.names = c("路段標識", "日期", "時間", "通行時間"), colClasses = c("character", "character", "character", "double"))
訓練表 = rbind(訓練表, fread(paste0(輸入目錄, "/gy_contest_traveltime_training_data_second.txt"), col.names = c("路段標識", "日期", "時間", "通行時間"), colClasses = c("character", "character", "character", "double"))[日期 < "2017-04-01"])
訓練表 = rbind(訓練表, fread(paste0(輸入目錄, "/gy_contest_link_traveltime_training_data.txt"), col.names = c("路段標識", "日期", "時間", "通行時間"), colClasses = c("character", "character", "character", "double")))
...
計算左係數表 = function(某訓練表, 某測試日序, 某訓練日序)
{
	...
	函式甲 = function(序號甲, 某選定測試表, 某測試左表, 某測試近左表, 某測試遠左表, 某訓練主表, 某測試日序, 某訓練日序, 取得曜日權重, 計算回歸值, 核數)
	{
		library(data.table)
	...
	左係數表清單 = parLapply(集群, 1:核數, 函式甲, 某選定測試表 = 某選定測試表, 某測試左表 = 某測試左表, 某測試近左表 = 某測試近左表, 某測試遠左表 = 某測試遠左表, 某訓練主表 = 某訓練主表, 某測試日序 = 某測試日序, 某訓練日序 = 某訓練日序, 取得曜日權重 = 取得曜日權重, 計算回歸值 = 計算回歸值, 核數 = 核數)
	stopCluster(集群)
...
計算左分序係數表 = function(某訓練表, 某測試日序, 某訓練日序)
{
	...
	集群 = makeCluster(getOption("cl.cores", 核數))
	左分序係數表清單 = parLapply(集群, 1:核數, 函式甲, 某選定測試表 = 某選定測試表, 某測試左表 = 某測試左表, 某測試近左表 = 某測試近左表, 某測試遠左表 = 某測試遠左表, 某訓練主表 = 某訓練主表, 某測試日序 = 某測試日序, 某訓練日序 = 某訓練日序, 取得曜日權重 = 取得曜日權重, 計算回歸值 = 計算回歸值, 核數 = 核數)
	stopCluster(集群)
...
預測 = function(某測試表, 某訓練表, 某左係數表, 某左分序係數表, 某測試日序, 某訓練日序)
{
	...
	函式甲 = function(序號甲, 某選定測試表, 某訓練主表, 某測試日序, 某訓練日序, 取得曜日權重, 計算回歸值, 核數)
	{
		library(data.table)
	...
	集群 = makeCluster(getOption("cl.cores", 核數))
	某主預測表清單 = parLapply(集群, 1:核數, 函式甲, 某選定測試表 = 某選定測試表, 某訓練主表 = 某訓練主表, 某測試日序 = 某測試日序, 某訓練日序 = 某訓練日序, 取得曜日權重 = 取得曜日權重, 計算回歸值 = 計算回歸值, 核數 = 核數)
	stopCluster(集群)
	...
	某預測表$近左係數 = 0.7 * (180 + 某預測表$基分序 - 某預測表$分序) * (0.06 + 0.7 * 某預測表$近左係數) / 60 + 0.3 * 某預測表$近左分序係數
	某預測表$近左係數[某預測表$近左預測 == 0] = 0
	某預測表$遠左係數 = 0.7 * (某預測表$分序 - 120 - 某預測表$基分序) * (0.06 + 0.7 * 某預測表$遠左係數) / 60 + 0.3 * 某預測表$遠左分序係數
...
write.table(預測表[, .(路段標識 = 路段標識, 日期 = sprintf("2017-07-%02d", 日序 + 1), 時間 = sprintf("[2017-07-%02d %02d:%02d:00,2017-07-%02d %02d:%02d:00)", 日序 + 1, 時, 分 - 1, 日序 + 1, 時 + (分 + 1) %/% 60, (分 + 1) %% 60), 通行時間 = 預測)], paste0(輸出目錄, "/result.txt"), row.names = F, quote = F, sep = "#")
